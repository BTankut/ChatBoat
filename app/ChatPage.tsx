...
export default function Home() {
  ...
  // Safe localStorage getter - handles Node.js v25 localStorage incompatibility
  const getStoredUrl = (): string | null => {
    ...
    try {
      ...
      if (storage && typeof storage.getItem === 'function') {
        return storage.getItem('aiServerUrl');
      }
    } catch {
      // localStorage not available or incompatible (Node.js v25 SSR)
    }
    return null;
  };

  // Safe localStorage setter - handles Node.js v25 localStorage incompatibility
  const setStoredUrl = (url: string): void => {
    if (typeof window === 'undefined' || typeof document === 'undefined') {
      return;
    }
    try {
      ...
      if (storage && typeof storage.setItem === 'function') {
        storage.setItem('aiServerUrl', url);
      }
    } catch {
      // localStorage not available or incompatible (Node.js v25 SSR)
    }
  ...
  useEffect(() => {
    ...
    if (serverUrl && serverUrl !== storedUrl) {
      refreshModels();
    }
  }, [serverUrl, mounted]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Check for /think or /no_think commands in the user input
    let userContent = input.trim();
    let messageThinkingMode = enableThinking; // Default to current global setting
    
    // /no_think ve /think komutlarını kontrol et, ancak içerikten çıkarma
    // LM Studio'nun kendi arayüzünde çalıştığı gibi, komutu doğrudan mesaj içeriğinde bırak
    if (userContent.endsWith("/think")) {
      messageThinkingMode = true;
    }
    
    if (userContent.endsWith("/no_think")) {
      messageThinkingMode = false;
    }
    ...
    try {
      ...
      const response = await fetch("/api/chat", {
        ...
        body: JSON.stringify({
          ...
          enableThinking: messageThinkingMode, // Send message-specific thinking preference
        }),
      });
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center ml-2 gap-2">
              ...
              <button 
                ...
                className="relative inline-block w-10 h-6 focus:outline-none focus:ring-2 focus:ring-blue-500 rounded-full"
                aria-pressed={enableThinking}
                aria-label="Toggle thinking mode"
              >
                <span 
                  className={`block w-10 h-6 rounded-full transition-colors ${enableThinking ? 'bg-blue-500' : 'bg-gray-300 dark:bg-gray-600'}`}
                ></span>
                <span 
                  className={`absolute left-1 top-1 bg-white w-4 h-4 rounded-full transition-transform transform ${enableThinking ? 'translate-x-4' : 'translate-x-0'}`}
                ></span>
              </button>
            </div>
          </div>
      ...
      {isSettingsOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 max-w-md w-full">
            ...
            <div className="mb-4 border-t pt-4 mt-4 dark:border-gray-700">
              <h3 className="text-md font-semibold mb-2">Model Settings</h3>
              
              <div className="flex items-center justify-between p-2 bg-gray-50 dark:bg-gray-700 rounded-lg">
                <div>
                  <p className="text-sm font-medium">Enable Thinking Process</p>
                  <p className="text-xs text-gray-500 dark:text-gray-400">
                    When enabled, the model will show its thinking process if supported.
                  </p>
                </div>
                
                <button 
                  onClick={() => setEnableThinking(!enableThinking)}
                  className="relative inline-block w-12 h-6 flex-shrink-0 focus:outline-none focus:ring-2 focus:ring-blue-500 rounded-full"
                  aria-pressed={enableThinking}
                  aria-label="Toggle thinking mode"
                >
                  <span 
                    className={`block w-12 h-6 rounded-full transition-colors ${enableThinking ? 'bg-blue-500' : 'bg-gray-300 dark:bg-gray-600'}`}
                  ></span>
                  <span 
                    className={`absolute left-1 top-1 bg-white w-4 h-4 rounded-full transition-transform transform ${enableThinking ? 'translate-x-6' : 'translate-x-0'}`}
                  ></span>
                </button>
              </div>
            </div>
